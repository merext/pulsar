...
impl Default for QuantumHftStrategy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl QuantumHftStrategy {
    ...
    #[allow(dead_code)]
    fn generate_micro_scalping_signal(&self, _current_price: f64) -> (Signal, f64) {
        ...
        let momentum_threshold = (volatility * 1.5).clamp(0.0002, 0.001);
        ...
        if short_momentum > momentum_threshold && velocity >= 0.0 && volatility > 0.0003 {
            debug!(
                "Micro Scalp Long: momentum={:.4}, velocity={:.4}, volatility={:.4}",
        ...
        if short_momentum < -momentum_threshold && velocity <= 0.0 && volatility > 0.0003 {
            debug!(
                "Micro Scalp Short: momentum={:.4}, velocity={:.4}, volatility={:.4}",
                short_momentum, velocity, volatility
            );
        ...
        if micro_trend == 1 && short_momentum > 0.0001 && velocity > -0.0001 {
            debug!(
                "Trend Scalp Long: trend={}, momentum={:.4}",
                micro_trend, short_momentum
            );
        ...
        if micro_trend == -1 && short_momentum < -0.0001 && velocity < 0.0001 {
            debug!(
                "Trend Scalp Short: trend={}, momentum={:.4}",
                micro_trend, short_momentum
            );
    ...
    #[allow(dead_code)]
    fn generate_contrarian_signal(&self, _current_price: f64) -> (Signal, f64) {
        if self.price_window.len() < 20 {
            return (Signal::Hold, 0.0);
        }
        ...
        if rsi > 75.0 && current_volume > self.avg_volume * 1.5 && momentum > 0.001 {
            debug!(
                "Contrarian Short: RSI={:.1}, volume={:.1}x, momentum={:.4}",
                rsi,
                current_volume / self.avg_volume,
                momentum
            );
        ...
        if momentum > 0.003 && rsi > 60.0 {
            debug!(
                "Quick Reversal Short: momentum={:.4}, RSI={:.1}",
                momentum, rsi
            );
            return (Signal::Sell, 0.8);
        }
        ...
        if momentum < -0.003 && rsi < 40.0 {
            debug!(
                "Quick Reversal Long: momentum={:.4}, RSI={:.1}",
                momentum, rsi
            );
            return (Signal::Buy, 0.8);
        }
    ...
    fn generate_volume_substrategy_signal(&self, current_price: f64) -> (Signal, f64) {
        ...
        if is_volume_spike && current_volume > self.avg_volume * 2.5 {
            if current_price > vwap * 1.005 && !near_resistance && momentum > 0.001 {
                debug!(
                    "Volume Breakout Long: price={:.4}, VWAP={:.4}, volume={:.1}x, momentum={:.4}",
                    current_price,
                    vwap,
                    current_volume / self.avg_volume,
                    momentum
            ...
            if current_price < vwap * 0.995 && !near_support && momentum < -0.001 {
                debug!(
                    "Volume Breakout Short: price={:.4}, VWAP={:.4}, volume={:.1}x, momentum={:.4}",
                    current_price,
                    vwap,
                    current_volume / self.avg_volume,
                    momentum
        ...
        if near_support
            && current_price < vwap * 0.998
            && current_volume > self.avg_volume * 1.8
            && momentum > 0.0005
        {
            debug!(
                "Support Bounce: price={:.4}, VWAP={:.4}, momentum={:.4}",
                current_price, vwap, momentum
            );
            return (Signal::Buy, 0.8);
        }

        if near_resistance
            && current_price > vwap * 1.002
            && current_volume > self.avg_volume * 1.8
            && momentum < -0.0005
        {
            debug!(
                "Resistance Bounce: price={:.4}, VWAP={:.4}, momentum={:.4}",
                current_price, vwap, momentum
            );
            return (Signal::Sell, 0.8);
        }
    ...
    fn generate_signal(&self, current_price: f64) -> (Signal, f64) {
        ...
        let rsi = self.calculate_rsi(&prices, 14);
        let (bb_upper, _bb_middle, bb_lower) = self.calculate_bollinger_bands(&prices, 20, 2.0);
        let momentum = self.calculate_momentum(&prices, 5);
        let sma_short = self.calculate_sma(&prices, 10);
        let sma_long = self.calculate_sma(&prices, 20);

        // Enhanced entry conditions
        let _bb_position = (current_price - bb_lower) / (bb_upper - bb_lower);
        ...
        if rsi > 99.9 && short_momentum < 0.001 {
            debug!(
                "Mean Rev Sell: RSI={:.1}, momentum={:.4}",
                rsi, short_momentum
            );
            return (Signal::Sell, 0.9);
        }
        ...
        if price_deviation < -0.002 && rsi < 45.0 {
            debug!(
                "Mean Reversion Buy: RSI={:.1}, Dev={:.4}",
                rsi, price_deviation
            );
        ...
        if price_deviation > 0.002 && rsi > 55.0 {
            debug!(
                "Mean Reversion Sell: RSI={:.1}, Dev={:.4}",
                rsi, price_deviation
            );
        ...
        if self.trade_counter % 100 == 0 {
            let recent_high = prices
                .iter()
                .rev()
                .take(20)
                .fold(f64::NEG_INFINITY, |a, &b| a.max(b));
            let recent_low = prices
                .iter()
                .rev()
                .take(20)
            ...
            if price_range > 0.0 {
                ...
                if position_in_range < 0.2 && momentum > -0.001 {
                    debug!(
                        "Range Bottom Buy: position={:.2}, range={:.4}",
                        position_in_range, price_range
                    );
                    return (Signal::Buy, 0.6);
                }
                ...
                if position_in_range > 0.8 && momentum < 0.001 {
                    debug!(
                        "Range Top Sell: position={:.2}, range={:.4}",
                        position_in_range, price_range
                    );
                    return (Signal::Sell, 0.6);
                }
...
impl Strategy for QuantumHftStrategy {
    fn get_info(&self) -> String {
        format!(
            "Profit-Optimized Multi-Strategy: Micro-Scalping + Volume + RSI+Momentum (Win Rate: {:.1}%, PnL: {:.4})",
            self.win_rate * 100.0,
            self.total_pnl
        )
    }
    ...
    fn get_signal(
        ...
        if current_position.quantity > 0.0 {
            ...
            if exit_signal == Signal::Sell || self.consecutive_losses >= 1 {
                debug!(
                    "Smart Long Exit: consecutive_losses={}",
            ...
            if momentum > 0.0002 || rsi > 52.0 {
                debug!(
                    "Conservative Long Profit: momentum={:.4}, RSI={:.1}",
                    momentum, rsi
                );
                return (Signal::Sell, 0.95);
            }
            ...
            if exit_signal == Signal::Buy || self.consecutive_losses >= 1 {
                debug!(
                    "Smart Short Exit: consecutive_losses={}",
                    self.consecutive_losses
                );
                return (Signal::Buy, 0.9);
            }
            ...
            if momentum < -profit_threshold * 0.5 || rsi < 48.0 - (volatility * 500.0) {
                debug!(
                    "Smart Short Profit: RSI={:.1}, Momentum={:.4}, Volatility={:.4}",
                    rsi, momentum, volatility
                );
            ...
            if momentum < -0.0002 || rsi < 48.0 {
                debug!(
                    "Conservative Short Profit: momentum={:.4}, RSI={:.1}",
                    momentum, rsi
                );
                return (Signal::Buy, 0.95);
            }
...
impl QuantumHftStrategy {
    pub fn on_trade_result(&mut self, result: f64) {
        ...
        debug!(
            "Trade Result: {:.4}, Win Rate: {:.1}%, Total PnL: {:.4}",
            result,
            self.win_rate * 100.0,
            self.total_pnl
        );
