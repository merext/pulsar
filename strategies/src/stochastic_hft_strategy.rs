...
use crate::strategy::{NoOpStrategyLogger, Strategy, StrategyLogger};
...
impl StochasticHftStrategy {
    ...
    #[must_use]
    pub fn new() -> Self {
        Self::from_file("config/stochastic_hft_strategy.toml")
...
impl StochasticHftStrategy {
    ...
    fn calculate_stochastic(&self) -> Option<(f64, f64)> {
        ...
        let prices: Vec<f64> = self
            ...
            .iter()
            .rev()
            .take(self.config.stochastic.k_period)
            .copied()
            .collect();
...
impl Strategy for StochasticHftStrategy {
    ...
    fn get_info(&self) -> String {
        ...
        format!(
            "{} - K: {:.1}, D: {:.1}",
            self.config.general.strategy_name, current_k, current_d
        )
    }
